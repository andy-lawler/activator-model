...
Tables useds:
                                Salescenter table:
                                axa_us_salescenter.policyfinandetail_img
                                axa_us_salescenter.brokerinfo
                               
                               
                                Discovery Historical Snapshots:
                                us_ia_pwc.discovery_insurance_all
                                us_ia_pwc.discovery_bdrep_all
                               
                               
                                Market Metrics:
                                us_ia_pwc.mm_raw_data
                                us_ia_pwc.mm_broker_key
                               
                               
                                Custom Tables:
                                us_ia_pwc.firm_friendliness
                                us_ia_pwc.disc_firm_map
                                us_ia_pwc.branch_friendliness
Abbreviations:
ins: refers to us_ia_pwc.discovery_insurance_all table
rbd: refers to us_ia_pwc.discovery_bdrep_all
ff: refers to us_ia_pwc.firm_friendliness
bf: refers to us_ia_pwc.branch_friendliness table
mm: refers to market metrics data
sc: refers to salescenter data
fma: refers to us_ia_pwc.disc_firm_map table
*/
...
We need two salescenter tables for this: axa_us_salescenter.policyfinandetail_img, axa_us_salescenter.brokerinfo
axa_us_salescenter.policyfinandetail_img -> this table gives us the sale dates of reps by brokerid
axa_us_salescenter.brokerinfo -> this table gives us the npn number of the reps
*/
...
(select
                ...
                from axa_us_salescenter.policyfinandetail_img a
                               
                                inner join axa_us_salescenter.brokerinfo b
...
Other and Unknown.*/
  ...
  */
    ...
    */
     ...
     (select *,substr(ss_date,1,7) as monthid,1 as is_in_insurance_table from axa_us_discovery.insurance )ins
      left join
        (select *,substr(ss_date,1,7) as monthid, 1 as is_in_bdrep_table from axa_us_discovery.bd_rep) rbd
               on ins.repcrd = rbd.repcrd
               and ins.monthid = rbd.monthid
                                               
                                                left join axa_us_discovery.bd_firm fbd
        ...
        */
        ...
        left join (select mm.*
                from
                    ...
                    from (
                        select a.*,b.firmcode from axa_us_salescenter.axa_raw_data a
                        left join (select fdsbrokerid,brokerid as firmcode from axa_us_salescenter.axa_broker_key group by 1,2) b
